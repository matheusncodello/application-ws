...
public class CampaignController {
    ...
    @ApiOperation(value = "Traz as campanhas mais recentes.")
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = "Realiza o método com sucesso."),
            @ApiResponse(code = 403, message = "Você não tem permissão para acessar este recurso."),
            @ApiResponse(code = 400, message = "Foi gerado um erro/exceção"),
    })
    @GetMapping(value = "/get-campaign-order")
    public DefaultResponseEnvelope<List<CampaignResponseDTO>> getCampaignOrderByRecents(){
        return new DefaultResponseEnvelope<>(
                true,
                campaignService.getPilha()
        );
    }

    @ApiOperation(value = "Traz as campanhas mais antigas.")
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = "Realiza o método com sucesso."),
            @ApiResponse(code = 403, message = "Você não tem permissão para acessar este recurso."),
            @ApiResponse(code = 400, message = "Foi gerado um erro/exceção"),
    })
    @GetMapping(value = "/get-campaign-order-oldest")
    public DefaultResponseEnvelope<Object[]> getCampaignOrderByOldest(){
        return new DefaultResponseEnvelope<>(
                true,
